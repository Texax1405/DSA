...
T Heap<T>::pop(){
    //YOUR CODE IS HERE
    if (this->count == 0){
        throw std::underflow_error("Calling pop on an empty heap.");
    }
    T removeItem = this->elements[0];
...
const T Heap<T>::peek(){
    //YOUR CODE IS HERE
    if (this->count == 0){
        throw std::underflow_error("Calling peek on an empty heap.");
    }
...
void Heap<T>::reheapDown(int position){
    ...
    if (leftChild < size() && aLTb(elements[leftChild],elements[largest])){
        largest = leftChild;
    } 

    if (rightChild < size() && aLTb(elements[rightChild], elements[largest])){
        largest = rightChild;
    }
