...
double_tensor MLPClassifier::predict(
    ...
    for(auto batch: *pLoader){
        ...
        double_tensor Y = this->forward(data);
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){
    ...
    for (auto batch : *pLoader)
    {
        double_tensor data = batch.getData();
        double_tensor label = batch.getLabel();

        double_tensor Y = this->forward(data);
        auto y_pred = xt::argmax(Y, 1);
        auto y_true = xt::argmax(label, 1);
...
void MLPClassifier::backward(){
    ...
    for (DLinkedList<ILayer *>::BWDIterator layer = m_layers.bbegin(); layer != m_layers.bend(); --layer){
        //cout << "begin in loop" << endl;
        ILayer *Layer = *layer;
