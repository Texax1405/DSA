...
//Destructor of class XArrayList
template <class T>
XArrayList<T>::~XArrayList()
{
    // TODO
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    {
        int new_capacity = this->capacity*2 + 1;
...
void XArrayList<T>::add(int index, T e)
{
  ...
  T* new_arr = new T [this->capacity];
   new_arr[index] = e;
    for (int i=0;i<index;++i) new_arr[i] = this->data[i];
    for (int i=index+1;i<=this->count;++i) new_arr[i] = this->data[i-1];
    delete[] this->data;
    this->data = new_arr;
    this->count++;
...
T XArrayList<T>::removeAt(int index)
{
    ...
    {
      new_arr[pos++] = this->data[i];
    }
    for (int i=index;i<this->count;++i)
    {
      new_arr[pos++] = this->data[i+1];
    }
...
int XArrayList<T>::indexOf(T item)
{
    ...
    for (int i=0;i<this->count;++i)
    {
        if (this->equals(data[i],item,this->itemEqual)) return i;
    }
    return -1;
}
template <class T>
bool XArrayList<T>::contains(T item)
{
    ...
    for (int i=0;i<count;++i)
    {
        if (this->equals(data[i],item,this->itemEqual)) return true;
    }
    return false;
}
