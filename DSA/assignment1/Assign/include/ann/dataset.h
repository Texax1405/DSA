...
public:
    ...
    */
    ...
    DataLabel<DType, LType> getitem(int index) {
        ...
        /*xt::xarray<DType> dataItem;
        if (data.shape().size() == 1){
            dataItem = xt::view(data, index); 
        } else if (data.shape().size() == 2 && data.shape()[1] == 1){
            dataItem = xt::view(data, index, xt::all());
        } else {
            dataItem = xt::view(data, index, xt::all(), xt::all());
        }

        xt::xarray<LType> labelItem;
        if (label.size() > 0){
            if (label.shape().size() == 1){
                labelItem = xt::view(label, index);
            } else {
                labelItem = xt::view(label, index, xt::all());
            }
